...
class TestGrobidPipeline(object):
    def test_should_pass_grobid_url_and_action_to_grobid_service(
        ...
        grobid_service.assert_called_with(
            args.grobid_url, args.grobid_action, start_service=False
        )
    ...
    def test_should_auto_start_grobid_service_if_grobid_url_is_none(
        ...
        _run_pipeline(config, args, PDF_INPUT)
        grobid_service.assert_called_with(
            LOCAL_GROBID_API_URL, args.grobid_action, start_service=True
        )
    ...
    def test_should_return_tei_content_as_xml_content_without_xslt(
        ...
        grobid_service_instance.return_value = (
    ...
    def test_should_pass_xslt_path_to_xslt_transform_from_file(
        ...
        _run_pipeline(config, args, PDF_INPUT)
        xslt_transformer_from_file.assert_called_with(
            args.grobid_xslt_path, pretty_print=True
        )
    ...
    def test_should_disable_pretty_print_xslt_output(
        ...
        _run_pipeline(config, args, PDF_INPUT)
        xslt_transformer_from_file.assert_called_with(
            args.grobid_xslt_path, pretty_print=False
        )
