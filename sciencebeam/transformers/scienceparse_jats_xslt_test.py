...
class TestScienceParseJatsXslt(object):
  class TestArticleTitle(object):
    ...
    def test_should_not_add_title_if_not_in_json(self, scienceparse_jats_xslt):
      jats = etree.fromstring(scienceparse_jats_xslt({}))
      assert jats.xpath('front/article-meta/title-group/article-title') == []
  ...
  class TestAuthor(object):
    def test_should_translate_single_author_with_first_and_last_name(self, scienceparse_jats_xslt):
      jats = etree.fromstring(scienceparse_jats_xslt({
        'authors': [{
    ...
    def test_should_translate_single_author_with_middle_name(self, scienceparse_jats_xslt):
      ...
      person = _get_item(jats, 'front/article-meta/contrib-group/contrib')
      assert _get_text(person, './name/given-names') == '%s %s' % (FIRST_NAME_1, MIDDLE_NAME_1)
      assert _get_text(person, './name/surname') == LAST_NAME_1
      ...
      jats = etree.fromstring(scienceparse_jats_xslt({
        'references': [
          extend_dict(REFERENCE_1, {
            'authors': [
              {'name': '%s %s' % (author['first-name'], author['last-name'])}
