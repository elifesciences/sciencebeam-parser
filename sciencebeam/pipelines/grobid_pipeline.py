...
HEADER_FIELDS = {FieldNames.TITLE, FieldNames.ABSTRACT, FieldNames.AUTHORS, FieldNames.AFFILIATIONS}
...
class GrobidPipeline(Pipeline):
  def add_arguments(self, parser, config, argv=None):
    ...
    grobid_group.add_argument(
      ...
      default=None,
      help='Name of the Grobid action (by default determined depending on the requested fields)'
    )
    grobid_group.add_argument(
  ...
  def get_steps(self, config, args):
    ...
    convert_to_tei = lambda pdf_filename, pdf_content, includes: call_grobid(
      (pdf_filename, pdf_content),
      path=args.grobid_action or get_default_grobid_action_for_fields(includes)
    )[1]
